...
def cluster_20min(package_model_config, data_query, exporter, verbose=False):
    ...
    pipeline = Pipeline([
        ...
        ('Partition', PartitionByTime(
            col='pca_eig0',
            partition_span=timedelta(minutes=20), exporter=exporter).after_transform(
            [
        ...
        ('KinkFinder', KinkFinderLabels(
            ...
            exporter=exporter)
         .after_transform([csv_save('model2_20min_kinkfinder')]))
    ])
...
def stepsize_pipeline(package_model_config, data_query, exporter, verbose=False):
    ...
    pipeline = Pipeline([
        ...
        ('Stepsize',StepSize(exporter=exporter, ignore_columns=["pca_eig"+str(i) for i in range(1,70,1)]).after_transform([csv_save_by_psn('model2_pca_stepsize')]))
    ])
...
def init_from_pkl(pkl_name, exporter):
    pipeline = Pipeline([ ('Stepsize',StepSize(exporter=exporter, ignore_columns=["pca_eig"+str(i) for i in range(1,70,1)]).after_transform([csv_save_by_psn('model2_pca_stepsize')]))])
